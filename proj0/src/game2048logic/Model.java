...
public class Model {
    ...
    public boolean atLeastOneMoveExists() {
        ...
        if (emptySpaceExists() || maxTileExists()) {
            ...
            for (int x = 0; x < board.size(); x++) {
                for (int y = 0; y < board.size(); y++) {
                    if (((x != board.size() - 1) && (y != board.size() - 1)) && (board.tile(x, y).value() == board.tile(x + 1, y).value() ||
                            board.tile(x, y).value() == board.tile(x, y + 1).value())) {
                        return true;
                    } else if (((x == board.size() - 1) && (y != board.size() - 1)) && (board.tile(x, y).value() == board.tile(x, y + 1).value())) {
                        return true;
                    } else if (((y == board.size() - 1) && (x != board.size() - 1)) && (board.tile(x, y).value() == board.tile(x + 1, y).value())) {
                        return true;
                    }
                }
            }
    ...
    public void moveTileUpAsFarAsPossible(int x, int y) {
        ...
        // The move method distinguish if the 'next' has the same value
        // So we just need to determine the targetY
    ...
    public void tilt(Side side) {
        ...
        // Tasks 8: just need to consider the up condition
